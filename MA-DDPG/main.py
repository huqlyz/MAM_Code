...
def assignment(policy, env_name, seed, eval_episodes, w1, w2, time, file_name):
    ...
    np.save(f"./results/{file_name}a1", evaluations1)
    np.save(f"./results/{file_name}a2", evaluations2)
    print("---------------------------------------------------------------------------------")
...
def evalpolicy(policy, env_name, seed, eval_episodes, f, time, file_name):
    ...
    if f == 1:
        ...
        for _ in range(eval_episodes):
            state, done = eval_env.reset(), False
            n = 0
            Ev = Ev + policy.estimate_value(state, max_action, 2)
            while not done:
                action = policy.select_action2(np.array(state))
                state, reward, done, _ = eval_env.step(action)
                avg_reward += reward
    ...
    #     file_object.write('\n')
    #     file_object.write(str(time)  + 'step \n')
    #     file_object.write('actor'+ str(f) +':' + str(avg) + '\n')
    #     file_object.write('C' + str(f) + 'Tv:' + str(Tv) + '  ')
    #     file_object.write('C' + str(f) + 'Ev:' + str(Ev) + '  \n')
    #     file_object.write('C' + str(f) + str((Tv - Ev) / Tv) + ' \n')
    #
    # return 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--M", default=3, type=int)  # environment number
    parser.add_argument("--assign", default=5e3,
                        type=int)  # How often (time steps) we Assign weights for generating experience
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if t < args.start_timesteps:
            ...
            if t % args.assign == 0 and t > args.start_timesteps:  # reassgin weight   #  # assign weight for generate experience
                if f == 0:
                    t1 = w1
                    t2 = w2
                    w1, w2 = assignment(policy, args.env, args.seed, 20, t1, t2, t, file_name)
                    w1 = int(w1)
                    w2 = int(w2)
                    print("fen pei ")
                    for i in range(args.M):
            ...
            for j in range(w2):  # actor1 generate exp
                i = j + w1
                #print(f'当前M编号{i}')
                action = (
        ...
        if t >= args.start_timesteps and t <= args.final_step:
            # policy.train(replay_buffer, args.batch_size)
            policy.train(replay_buffer, args.batch_size, 1)  #
            policy.train(replay_buffer, args.batch_size, 2)  #

        if t > args.final_step and f == 0:
            f = assignment_final(policy, args.env, args.seed, 50)
        elif f == 1:
            w1 = 1
            w2 = 0
            policy.train(replay_buffer,args.batch_size, 1)
    ...
    if f == 1:
        ...
        filename = './results/' + f"{args.policy}_{args.env}_{args.seed}" + '.cvs'
